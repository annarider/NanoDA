...
of all the names. If there is only one name, the list will have only one item in it, if the name is "NULL",
...
def fix_name(name):
    ...
    # check if the name value is an array with curly braces
    if '{' in name or '}' in name:
        # remove all curly braces
        name_list = re.sub('[{}]', '', name)
        # once curly braces are removed, add each name as item in list
