...
def extract_carriers(page):
    ...
    with open(page, "r") as html:
        # read in html & make it a beautiful soup object
        soup = BeautifulSoup(html)

        # find all the codes from the options tag (carrier & airport)
        codes = soup.find_all('option')
        # iterate through codes & discard the combinations
        # filter codes by selecting only 2 char codes; append to list
        for code in codes:
            if len(code['value']) == 2: 
